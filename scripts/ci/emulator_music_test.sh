#!/usr/bin/env bash
# Logcat smoke test for the music smoke APK, running inside the `smoke` compose container.
#
# Verifies the android-notify-music-bridge AAR is resolved from Maven (fixed package
# org.android_notify.music), that the media session notification is built and playing, and
# that injected KEYCODE_MEDIA_* events reach the Java MediaSession callback.
set -euo pipefail
...
_logcat_contains() {
  $ADB logcat -d -v brief 2>/dev/null | grep -q "$1"
}

_fail() {
  echo "FAIL: $1" >&2
  echo "---- last logcat (grep m)/music/python) ----" >&2
  $ADB logcat -d -v brief 2>/dev/null | grep -iE 'MusicNotification|MUSIC_|MediaSession|python|AndroidRuntime' | tail -60 >&2 || true
  exit 1
}
...
while [ "$_elapsed" -lt "$LOG_TIMEOUT" ]; do
  if _logcat_contains "MUSIC_BRIDGE_OK" && _logcat_contains "MUSIC_NOTIFICATION_BUILT" && _logcat_contains "MUSIC_PLAYING"; then
  ...
  echo "    … waiting for startup markers (${_elapsed}s / ${LOG_TIMEOUT}s)"
  sleep 5
  _elapsed=$((_elapsed + 5))
done

if ! _logcat_contains "MUSIC_BRIDGE_OK"; then
  _fail "MUSIC_BRIDGE_OK not found (AAR class org/android_notify/music/MediaSessionCallback not resolved)"
fi
...
if ! _logcat_contains "MUSIC_STATE: pause"; then
...
$ADB shell input keyevent 85 >/dev/null
if ! _logcat_contains "MUSIC_STATE: play"; then
  _fail "resume not detected after KEYCODE_MEDIA_PLAY_PAUSE"
fi
echo "==> Play detected"
...
sleep 1
if ! _logcat_contains "MUSIC_SKIPPED"; then
