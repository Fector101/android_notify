...
class SmokeApp(App):
    ...
    def build(self):
        return Label(
            text="android-notify music smoke",
            color=(0, 0, 0, 1),
        )
    ...
    def on_start(self):
        try:
            if MediaSessionCallback is not None:
                print(
                    "MUSIC_BRIDGE_OK: "
                    "org.android_notify.music.MediaSessionCallback loaded",
                    flush=True,
                )
            else:
                print(
                    "MUSIC_BRIDGE_MISSING: "
                    "MediaSessionCallback was not resolved",
                    flush=True,
                )
    ...
    def _run_smoke(self):
        ...
        self.mn = MusicNotification(
            on_next=lambda: print(
                "MUSIC_SKIPPED: next",
                flush=True,
            ),
            on_previous=lambda: print(
                "MUSIC_SKIPPED: prev",
                flush=True,
            ),
        )
    ...
    def _smoke_timeout(self, dt):
        ...
        if not self._first_play_printed:
            print(
                "MUSIC_PLAYING_TIMEOUT",
                flush=True,
            )

        if not (
            self._built_printed
            and self._first_play_printed
        ):
            self.stop()
